...
def run_sim(param_actual):
    ...
    N_J, N_Y, N_A = abc_example.species1(param_actual,temp1, T_FINAL, no_patches,N_J0, N_Y0,N_A0)
    ...
    for i in range(0,NUMBER_SIMS):
        ...
        width_theta  =np.random.uniform(1,5)#np.random.normal(2,1)
        ...
        N_J_sim, N_Y_sim, N_A_sim = abc_example.species1(PARAMS_ABC,temp1, T_FINAL, no_patches,N_J0, N_Y0,N_A0)
...
def actual_params(param):
    param["g_J"]   = np.random.uniform(0,0.7)#np.random.normal(0.4,0.3) #np.random.beta(2,2)
    param["g_Y"]    =np.random.uniform(0,0.7) #np.random.uniform(0,1)#np.random.beta(2,2)
    param["Topt"]  =np.random.uniform(1,9)#np.random.normal(6.5,2) #np.random.uniform(1,12) #np.random.lognormal(1,1)
    param["width"]  =np.random.uniform(1,20)#np.random.normal(2,1)
    ##np.random.lognormal(1,1)
    param["kopt"]     =np.random.uniform(0,0.7)#np.random.normal(0.5,0.4)# np.random.u(0,1)
    param["xi"]     =np.random.uniform(0,0.5/4)#np.random.normal(0.1,0.09) #np.random.normal(0,1)#np.random.normal(0,0.5)
...
#normalize the rows of Obs_sim to have NOS in row 1 and NSS in the remaining rows. Substract rows i=2:NUMBER_SIMS from row 1 of Obs_sim (whic contain OS).Compute the eucleadean distance (d) between NSS and NOS then use it along side tolerance (δ), to determine all parameters and NSS corresponding to d ≤ δ.Choose δ such that δ × 100% of the NUMBER_SIMS simulated parameters and NSS are selected. retain the parameters that made this threshold (library), the weights ot be used in local linear regression and the NSS that meets the threshold (stats)
        ...
        N_J1, N_Y1, N_A1 = abc_example.species1(param_actual,temperatures, T_forcast, no_patches,N_J0, N_Y0,N_A0)
        N_J2, N_Y2, N_A2 = abc_example.species1(PARAMS2,temperatures, T_forcast, no_patches,N_J0, N_Y0,N_A0)
        for q in range(1,no_patches+1):
