...
def simulation_population(params, time_general, temp):
    ...
    for t in range(0,rows-1):
        #boundary values
        N_J[t+1,0]=max(0,(1-params["m_J"])*N_J[t,0]-params["g_J"]*N_J[t,0]+(1-params["xi"])*N_A[t,0]*np.exp(alpha[t,0]*(1-(N_A[t,0]+N_J[t,0]+N_Y[t,0])/params["K"]))+ params["xi"]*N_A[t,1]*np.exp(alpha[t,1]*(1-(N_A[t,1]+N_Y[t,1]+N_J[t,1])/params["K"])))
        N_J[t+1,cols-1]=max(0,(1-params["m_J"])*N_J[t,cols-1]-params["g_J"]*N_J[t,cols-1]+(1-params["xi"])*N_A[t,cols-1]*np.exp(alpha[t,cols-1]*(1-(N_A[t,cols-1]+N_Y[t,cols-1]+N_J[t,cols-1])/params["K"]))+ params["xi"]*N_A[t,cols-2]*np.exp(alpha[t,cols-2]*(1-(N_A[t,cols-2]+N_Y[t,cols-2]+N_J[t,cols-2])/params["K"])))
        ...
        for x in range(1, cols-1):
            N_J[t+1,x]=max(0,(1-params["m_J"])*N_J[t,x]-params["g_J"]*N_J[t,x]+(1-2*params["xi"])*N_A[t,x]*np.exp(alpha[t,x]*(1-(N_A[t,x]+N_Y[t,x]+N_J[t,x])/params["K"]))+ params["xi"]*N_A[t,x+1]*np.exp(alpha[t,x+1]*(1-(N_A[t,x+1]+N_Y[t,x+1]+N_J[t,x+1])/params["K"]))+ params["xi"]*N_A[t,x-1]*np.exp(alpha[t,x-1]*(1-(N_A[t,x-1]+N_Y[t,x-1]+N_J[t,x-1])/params["K"])))
