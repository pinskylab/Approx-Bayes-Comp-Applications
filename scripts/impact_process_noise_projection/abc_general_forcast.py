...
def simulation_population(params, time_general, temp):
    ...
    for t in range(0,rows-1):
        #boundary values
        N_J[t+1,0]=np.random.poisson(lam=max(0,(1-params["m_J"])*N_J[t,0]-params["g_J"]*N_J[t,0]+(1-params["xi"])*N_A[t,0]*np.exp(alpha[t,0]*(1-(N_A[t,0]+N_Y[t,0]+N_J[t,0])/params["K"]))+ params["xi"]*N_A[t,1]*np.exp(alpha[t,1]*(1-(N_A[t,1]+N_Y[t,1]+N_J[t,1])/params["K"]))))
        N_J[t+1,cols-1]=np.random.poisson(lam=max(0,(1-params["m_J"])*N_J[t,cols-1]-params["g_J"]*N_J[t,cols-1]+(1-params["xi"])*N_A[t,cols-1]*np.exp(alpha[t,cols-1]*(1-(N_A[t,cols-1]+N_Y[t,cols-1]+N_J[t,cols-1])/params["K"]))+ params["xi"]*N_A[t,cols-2]*np.exp(alpha[t,cols-2]*(1-(N_A[t,cols-2]+N_Y[t,cols-2]+N_J[t,cols-2])/params["K"]))))
        N_Y[t+1, 0] = np.random.poisson(lam=max(0,(1-params["m_Y"])*N_Y[t,0]+params["g_J"]*N_J[t,0]-params["g_Y"]*N_Y[t,0]))
        N_Y[t+1, cols-1] = np.random.poisson(lam=max(0,(1-params["m_Y"])*N_Y[t,cols-1]+params["g_J"]*N_J[t,cols-1]-params["g_Y"]*N_Y[t,cols-1]))
        N_A[t+1, 0]=np.random.poisson(lam=max(0,(1-params["m_A"])*N_A[t,0]+params["g_Y"]*N_Y[t,0]))
        N_A[t+1, cols-1]=np.random.poisson(lam=max(0,(1-params["m_A"])*N_A[t,cols-1]+params["g_Y"]*N_Y[t,cols-1]))
        for x in range(1, cols-1):
            N_J[t+1,x]=np.random.poisson(lam=max(0,(1-params["m_J"])*N_J[t,x]-params["g_J"]*N_J[t,x]+(1-2*params["xi"])*N_A[t,x]*np.exp(alpha[t,x]*(1-(N_A[t,x]+N_Y[t,x]+N_J[t,x])/params["K"]))+ params["xi"]*N_A[t,x+1]*np.exp(alpha[t,x+1]*(1-(N_A[t,x+1]+N_Y[t,x+1]+N_J[t,x+1])/params["K"]))+ params["xi"]*N_A[t,x-1]*np.exp(alpha[t,x-1]*(1-(N_A[t,x-1]+N_Y[t,x-1]+N_J[t,x-1])/params["K"]))))
            N_Y[t+1, x] = np.random.poisson(lam=max(0,(1-params["m_Y"])*N_Y[t,x]+params["g_J"]*N_J[t,x]-params["g_Y"]*N_Y[t,x]))
            N_A[t+1, x]=np.random.poisson(lam=max(0,(1-params["m_A"])*N_A[t,x]+params["g_Y"]*N_Y[t,x]))
    return N_J, N_Y, N_A
...
def run_sim(param_actual):
    ...
    N_J, N_Y, N_A = simulation_population(param_actual, T_FINAL, temp1)
...
#normalize the rows of Obs_sim to have NOS in row 1 and NSS in the remaining rows. Substract rows i=2:NUMBER_SIMS from row 1 of Obs_sim (whic contain OS).Compute the eucleadean distance (d) between NSS and NOS then use it along side tolerance (δ), to determine all parameters and NSS corresponding to d ≤ δ.Choose δ such that δ × 100% of the NUMBER_SIMS simulated parameters and NSS are selected. retain the parameters that made this threshold (library), the weights ot be used in local linear regression and the NSS that meets the threshold (stats)
        ...
        PARAMS1  = {}
        PARAMS1 = {"g_J": param_actual["g_J"], "g_Y": param_actual["g_Y"], "Topt":param_actual["Topt"], "width":param_actual["width"], "kopt": param_actual["kopt"],"xi":param_actual["xi"], "m_J": param_actual["m_J"], "m_Y": param_actual["m_Y"], "m_A": param_actual["m_A"], "delta_t": param_actual["delta_t"], "K":param_actual["K"]}
        PARAMS2={}
        PARAMS2 = {"g_J": result[0], "g_Y": result[1], "Topt":result[2], "width":result[3], "kopt": result[4],"xi":result[5], "m_J": result[6], "m_Y": result[7], "m_A": result[8], "delta_t": param_actual["delta_t"], "K":result[9]}
        N_J1, N_Y1, N_A1 = simulation_population( PARAMS1, T_forcast, temperatures)
        N_J2, N_Y2, N_A2 = simulation_population( PARAMS2, T_forcast, temperatures)
        for q in range(1,no_patches+1):
