...
def run_sim(param_actual3):
    ...
    N_J, N_Y, N_A = abc_example.species3(param_actual3,temp1, T_FINAL, no_patches,N_J0, N_Y0,N_A0)
    ...
    for i in range(0,NUMBER_SIMS):
        ...
        #N_J_sim, N_Y_sim, N_A_sim = simulation_population(PARAMS_ABC, T_FINAL, temp1) # simulates population with g_J value
        N_J_sim, N_Y_sim, N_A_sim = abc_example.species2(PARAMS_ABC, temp1, T_FINAL, no_patches,N_J0, N_Y0,N_A0)
...
def compute_weight(kernel,t, eps, index):
     ...
     return weights
############################################################################################
def actual_params2(PARAMS2):
    PARAMS2["g_J"]   = np.random.uniform(0,0.7)#np.random.normal(0.4,0.3) #np.random.beta(2,2)
    PARAMS2["g_Y"]    =np.random.uniform(0,0.7) #np.random.uniform(0,1)#np.random.beta(2,2)
    PARAMS2["Topt"]  =np.random.uniform(3,6#np.random.normal(6.5,2) #np.random.uniform(1,12) #np.random.lognormal(1,1)
    PARAMS2["width"]  =np.random.uniform(1,5)#np.random.normal(2,1)
    ##np.random.lognormal(1,1)
    PARAMS2["kopt"]     =np.random.uniform(0,0.7)#np.random.normal(0.5,0.4)# np.random.u(0,1)
    PARAMS2["xi"]     =np.random.uniform(0,0.5/4)#np.random.normal(0.1,0.09) #np.random.normal(0,1)#np.random.normal(0,0.5)
    PARAMS2["m_J"]     =np.random.uniform(0,0.7)#np.random.normal(0.04,0.04) # #np.random.beta(2,2)
    PARAMS2["m_Y"]   =np.random.uniform(0,0.7)#np.random.normal(0.05,0.04) #np.random.uniform(0,1) #np.random.beta(2,2)
    PARAMS2["m_A"]   =np.random.uniform(0,0.7)#np.random.normal(0.05,0.05)# np.random.uniform(0,1)#np.random.beta(2,2)
    PARAMS2["delta_t"]   =0.1
    PARAMS2["K"]   =np.random.uniform(500,2000)
    return PARAMS2
##########################################################################################
def actual_params3(PARAMS3):
    PARAMS3["L_0"]   = np.random.uniform(0,3)#np.random.normal(0.4,0.3) #np.random.beta(2,2)
    PARAMS3["L_inf"]    =np.random.uniform(17,23) #np.random.uniform(0,1)#np.random.beta(2,2)
    PARAMS3["L_J"]    =5
    PARAMS3["L_Y"]    =10
    PARAMS3["Topt"]  =np.random.uniform(3,6)#np.random.normal(6.5,2) #np.random.uniform(1,12) #np.random.lognormal(1,1)
    PARAMS3["width"]  =np.random.uniform(1,5)#np.random.normal(2,1)
    ##np.random.lognormal(1,1)
    PARAMS3["kopt"]     =np.random.uniform(0,0.7)#np.random.normal(0.5,0.4)# np.random.u(0,1)
    PARAMS3["xi"]     =np.random.uniform(0,0.5/4)#np.random.normal(0.1,0.09)
    PARAMS3["r"]=np.random.uniform(0,0.7)
    PARAMS3["m_J"]     =np.random.uniform(0,0.7)#np.random.normal(0.04,0.04) # #np.random.beta(2,2)
    PARAMS3["m_Y"]   =np.random.uniform(0,0.7)#np.random.normal(0.05,0.04) #np.random.uniform(0,1) #np.random.beta(2,2)
    PARAMS3["m_A"]   =np.random.uniform(0,0.7)#np.random.normal(0.05,0.05)# np.random.uniform(0,1)#np.random.beta(2,2)
    PARAMS3["delta_t"]   =0.1
    PARAMS3["K"]   =np.random.uniform(500,2000)
    return PARAMS3
##########################################################################################
...
if __name__ == '__main__':
    ...
    PARAMS2 ={} #{"g_J": 0.4, "g_Y": 0.3, "Topt": 5, "width": 2, "kopt": 0.6,"xi":0.1, "m_J": .05, "m_Y": .05, "m_A": .05, "delta_t": 0.1, "K":100}
    PARAMS3 ={} #{"L_0": 1, "L_inf": 20,"L_J": 5,"L_Y": 10, "Topt": 6.5, "width": 2, "kopt": 0.6,"xi":0.1,"r":0.3, "m_J": .04, "m_Y": .05, "m_A": .05, "delta_t": 0.1, "K":100}
    #final time
    ...
    alpha= temperatures.copy()
    temperatures[:, 0]=np.linspace(0, temp_max,T_forcast)
    for x in range(1, cols):
        temperatures[:, x]=temperatures[:, x-1]+0.1
...
#temp1=temperatures
    ...
    for i in range(N_Species ):
        param_actual2=actual_params2(PARAMS2)
        param_actual3=actual_params3(PARAMS3)
    #############################################################################
        param_save, Obs_Sim         = run_sim(param_actual3)
    ######################################################################################################
        library, dists, stats,stats_SS,  NSS_cutoff, library_index   = sum_stats(Obs_Sim, param_save)
        #result2 =do_rejection(library2)
        result =do_rejection(library)
        PARAMS2_1={}
        PARAMS2_1 = {"g_J": result[0], "g_Y": result[1], "Topt":result[2], "width":result[3], "kopt": result[4],"xi":result[5], "m_J": result[6], "m_Y": result[7], "m_A": result[8], "delta_t": param_actual2["delta_t"], "K":result[9]}
        N_J1, N_Y1, N_A1 = abc_example.species2(param_actual2, temperatures, T_forcast, no_patches,N_J0, N_Y0,N_A0)
        N_J2, N_Y2, N_A2 = abc_example.species2(PARAMS2_1, temperatures, T_forcast, no_patches,N_J0, N_Y0,N_A0)
        for q in range(1,no_patches+1):
    ...
    import plot
    for q in range(1,no_patches+1):
        plot.do_forcast(D['A_bias'+ str(q)],temperatures, time, 'A_bias_mis'+ str(q))
        plot.do_forcast(D['Y_bias'+ str(q)],temperatures, time, 'Y_bias_mis'+ str(q))
        plot.do_forcast(D['J_bias'+ str(q)],temperatures, time, 'J_bias_mis'+ str(q))
