...
def species1(params, temperatures, T_FINAL, no_patches, N_J0, N_Y0,N_A0):
    ...
    for t in range(0,rows-1):
        #boundary values
        N_J[t+1,0]=max(0,(1-params["m_J"])*N_J[t,0]-params["g_J"]*N_J[t,0]+N_A[t,0]*np.exp(alpha[t,0]*(1-(N_A[t,0]+N_Y[t,0]+N_J[t,0])/params["K"])))
        N_J[t+1,cols-1]=max(0,(1-params["m_J"])*N_J[t,cols-1]-params["g_J"]*N_J[t,cols-1]+N_A[t,cols-1]*np.exp(alpha[t,cols-1]*(1-(N_A[t,cols-1]+N_Y[t,cols-1]+N_J[t,cols-1])/params["K"])))
        ...
        for x in range(1, cols-1):
            N_J[t+1,x]=max(0,(1-params["m_J"])*N_J[t,x]-params["g_J"]*N_J[t,x]+N_A[t,x]*np.exp(alpha[t,x]*(1-(N_A[t,x]+N_Y[t,x]+N_J[t,x])/params["K"])))
