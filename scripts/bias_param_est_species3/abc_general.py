...
def run_sim(param_actual):
    ...
    N_J, N_Y, N_A = abc_example.species3(param_actual,temperatures, T_FINAL, no_patches,N_J0, N_Y0,N_A0)
# N_J, N_Y, N_A = simulation_population(param_actual,temperatures)
    ...
    for i in range(0,NUMBER_SIMS):
        ...
        N_J_sim, N_Y_sim, N_A_sim = abc_example.species3(PARAMS_ABC, temperatures, T_FINAL, no_patches,N_J0, N_Y0,N_A0) # simulates population with g_J value
...
def actual_params(PARAMS):
    PARAMS["L_0"]   = np.random.uniform(0,3)#np.random.normal(0.4,0.3) #np.random.beta(2,2)
    PARAMS["L_inf"]    =np.random.uniform(17,23) #np.random.uniform(0,1)#np.random.beta(2,2)
    PARAMS["L_J"]    =5
    PARAMS["L_Y"]    =10
    ...
    PARAMS["xi"]     =np.random.uniform(0,0.5/4)#np.random.normal(0.1,0.09)
    PARAMS["r"]=np.random.uniform(0,0.7)
...
if __name__ == '__main__':
    ...
    #final time
    param_bound=np.array([[0,3],[18,23],[5,9],[1,4],[0,1],[0,0.5/2],[0,1],[0,1],[0,1],[0,1],[100, 500]])
    T_FINAL = 30
    #number of patches
    ...
    for i in range(N_Species ):
        ...
        actual=[param_actual["L_0"], param_actual["L_inf"], param_actual["Topt"], param_actual["width"], param_actual["kopt"],param_actual["xi"],param_actual["r"], param_actual["m_J"], param_actual["m_Y"], param_actual["m_A"], param_actual["K"]]
    #############################################################################
...
#normalize the rows of Obs_sim to have NOS in row 1 and NSS in the remaining rows. Substract rows i=2:NUMBER_SIMS from row 1 of Obs_sim (whic contain OS).Compute the eucleadean distance (d) between NSS and NOS then use it along side tolerance (δ), to determine all parameters and NSS corresponding to d ≤ δ.Choose δ such that δ × 100% of the NUMBER_SIMS simulated parameters and NSS are selected. retain the parameters that made this threshold (library), the weights ot be used in local linear regression and the NSS that meets the threshold (stats)
        ...
        coverage_rej,resultsbias_rej=do_goodness_fit(result_rej,HPDR_rej, actual, len(PARAMS)-3, i)
    ...
    print("The Coverage probability of species 3 from rejection  is:", coverage_rej_percen)
#print("The Coverage probabilityfrom regression  is:", coverage_reg_percen)
    ...
    box_plot_rej.savefig('box_plot_rej_10patches_species3.png', bbox_inches='tight')
    plt.close()
